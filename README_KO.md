...
[WinAFL](https://github.com/googleprojectzero/winafl) 퍼징 결과 크래시 파일을 자동으로 디버깅하여 재현 여부 확인, 크래시 지점 분류, 중복 그룹화를 수행하는 자동화 도구.

## 개발 배경

WinAFL을 활용한 퍼징 수행 후, 생성된 크래시 파일들에 대해 다음 작업을 수동으로 진행해야 한다.

1. **재현 확인** — 크래시 파일을 디버거로 재실행하여 실제 크래시 발생 여부 검증
2. **크래시 지점 식별** — 예외 발생 위치(`module!function+offset`) 및 예외 유형 확인
3. **중복 제거** — 동일한 크래시 지점에서 발생한 파일들을 그룹화하여 분류

그러나 퍼징 결과물로 생성되는 크래시 파일은 통상 100개 이상으로, 이를 사람이 건별로 수동 처리하기에는 시간과 자원 소모가 과도하다. 이러한 반복 작업의 비효율을 해소하고자 Claude Code를 활용하여 전 과정을 자동화하는 스크립트를 개발하였다.
